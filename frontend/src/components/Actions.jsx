...
const Actions = ({ post }) => {
    ...
    const [postAction, setPostAction] = useRecoilState(postsAtom)
    ...
    const handleLike = async () => {
        if (!user) return toast({ title: "Hint", description: "You Must Login To do Action ", status: "info", isClosable: true })
        if (isLiking) return;
        setIsLiking(true)
        try {
            ...
            if (data.error) {
                toast({
                    title: "Error", isClosable: true, description: data.error, status: "error", duration: 3000
                })
            }
            const updatedPost = {
                ...post,
